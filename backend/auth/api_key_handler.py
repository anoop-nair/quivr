...
async def verify_api_key(api_key: str, commons: CommonsDep):     
    try:
        ...
        result = get_datastore_client().table('api_keys').select('api_key', 'creation_time').filter('api_key', 'eq', api_key).filter('is_active', 'eq', True).execute()
...
async def get_user_from_api_key(api_key: str, commons: CommonsDep):
    # Lookup the user_id from the api_keys table
        user_id_data = get_datastore_client().table('api_keys').select('user_id').filter('api_key', 'eq', api_key).execute()
        ...
        user_email_data = get_datastore_client().table('users').select('email').filter('user_id', 'eq', user_id).execute()
