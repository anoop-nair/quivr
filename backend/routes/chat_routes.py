...
def get_user_chats(user_id):
    response = get_datastore_client().from_('chats').select('chatId:chat_id, chatName:chat_name').filter("user_id", "eq", user_id).execute()
    return response.data

def get_chat_details(chat_id):
    response = get_datastore_client().from_('chats').select('*').filter("chat_id", "eq", chat_id).execute()
    return response.data

def delete_chat_from_db(commons, chat_id):
    get_datastore_client().table("chats").delete().match({"chat_id": chat_id}).execute()

def fetch_user_stats(commons, user, date):
    response = get_datastore_client().from_('users').select('*').filter("email", "eq", user.email).filter("date", "eq", date).execute()
...
async def get_chats(chat_id: UUID):
    ...
    """
    chats = get_chat_details(chat_id)
    if len(chats) > 0:
...
def chat_handler(request, commons, chat_id, chat_message, email, is_new_chat=False):
    ...
    user_id = fetch_user_id_from_credentials({"email": email})
    max_requests_number = os.getenv("MAX_REQUESTS_NUMBER")
    user_openai_api_key = request.headers.get('Openai-Api-Key')

    userItem = fetch_user_stats(User(email=email), date)
    ...
    if old_request_count == 0: 
        ...
        update_user_request_count(email, date, requests_count=old_request_count + 1)
