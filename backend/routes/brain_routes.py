...
def get_user_brains(user_id):
    response = get_datastore_client().from_('brains_users') \
...
def get_brain(brain_id):
    response = get_datastore_client().from_('brains').select('brainId:brain_id, brainName:brain_name').filter("brain_id", "eq", brain_id).execute()
    return response.data

def get_brain_details(brain_id):
    response = get_datastore_client().from_('brains').select('*').filter("brain_id", "eq", brain_id).execute()
    return response.data

def delete_brain(brain_id):
    # Does it also delete it in brains_users and brains_vectors ? 
    get_datastore_client().table("brains").delete().match({"brain_id": brain_id}).execute()

def create_brain(brain = Brain):
   response = get_datastore_client().table("brains").insert(
        brain).execute() 
   return response.data

def create_brain_user(brain_id, user_id, rights):
   response = get_datastore_client().table("brains_users").insert(
        { "brain_id": brain_id, "user_id": user_id, "rights": rights}).execute() 
   return response.data

def create_brain_vector(brain_id, vector_id):
   response = get_datastore_client().table("brains_users").insert(
        { "brain_id": brain_id, "vector_id": vector_id}).execute() 
   return response.data

def get_vector_ids_from_file_sha1(file_sha1: str):
    vectorsResponse = get_datastore_client().table("vectors").select("id").filter("metadata->>file_sha1", "eq", file_sha1) \
        .execute()
    print('vectorsResponse', vectorsResponse.data)
    return vectorsResponse.data

def update_brain_fields(brain: BrainToUpdate):
    # Need to only get the not undefined brain fields passed Optional['Brain'] -> create a BrainToUpdate type 
    get_datastore_client().table("brains").update(
        { "brain_name": brain.brain_name}).match({"brain_id": brain.brain_id}).execute()
    logger.info(f"Brain {brain.brain_id} updated")

def update_brain_with_file(brain_id:UUID , file_sha1: str ):
        # add all the vector Ids to the brains_vectors  with the given brain.brain_id
        vector_ids = get_vector_ids_from_file_sha1(file_sha1)
        for vector_id in vector_ids: 
            create_brain_vector(brain_id=brain_id, vector_id = vector_id)
...
async def brain_endpoint(brain_id: UUID):
    ...
    """
    brains = get_brain_details(brain_id)
    if len(brains) > 0:
...
async def brain_endpoint(request: Request, brain: Brain, current_user: User = Depends(get_current_user)):
    ...
    user_id = fetch_user_id_from_credentials({"email": current_user.email})
    created_brain = create_brain(brain)
    # create a brain 
...
async def brain_endpoint(request: Request, brain_id: UUID, brain: BrainToUpdate, fileName: Optional[str], current_user: User = Depends(get_current_user)):
    ...
    if brain.file_sha1 : 
        # add all the vector Ids to the brains_vectors  with the given brain.brain_id
        update_brain_with_file(brain_id= brain.brain_id, file_sha1=brain.brain_name)
        print("brain:", brain)

    update_brain_fields(brain)
