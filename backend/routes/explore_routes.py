...
def get_unique_user_data(user):
    ...
    """
    response = get_datastore_client().table("vectors").select(
...
async def explore_endpoint(current_user: User = Depends(get_current_user)):
    ...
    """
    unique_data = get_unique_user_data(current_user)
...
async def delete_endpoint(file_name: str, credentials: dict = Depends(AuthBearer())):
    ...
    get_datastore_client().table("summaries").delete().match(
        {"metadata->>file_name": file_name}).execute()
    get_datastore_client().table("vectors").delete().match(
